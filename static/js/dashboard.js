...
function updateProfileUI(name, role) {
    ...
    const initMobile = document.getElementById('user-initials-mobile');
    const letter = name.charAt(0).toUpperCase();

    if (nameEl) nameEl.innerText = name;
    if (roleEl) roleEl.innerText = role;
    if (initEl) initEl.innerText = letter;
    if (initMobile) initMobile.innerText = letter;
}
...
function switchTab(tabName) {
    const sections = ['writer', 'bulk', 'history', 'analytics'];
    ...
    sections.forEach(s => {
        ...
        if (sectionEl && btnEl) {
            ...
            btnEl.classList.remove('nav-active');
        }
    });
    ...
    if (activeSection && activeBtn) {
        ...
        if (window.NeuralUI?.staggerSection) {
            window.NeuralUI.staggerSection(activeSection.querySelector('.stagger-children') || activeSection);
        }
        if (typeof gsap !== 'undefined') {
            gsap.fromTo(activeSection, { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 0.55, ease: 'power3.out' });
        }
    }
    ...
    if (tabName === 'bulk' && window.BulkSend) {
        window.BulkSend.loadDraftFromStorage();
    }
}

function useDraftInBulkSend(content) {
    if (window.BulkSend) {
        window.BulkSend.applyDraftFromContent(content);
    }
    switchTab('bulk');
            ...
            if (tableBody) {
                if (data.history && data.history.length > 0) {
                    tableBody.innerHTML = data.history.map(row => {
                        const safeContent = (row.content || '').replace(/\\/g, '\\\\').replace(/`/g, '\\`').replace(/'/g, "\\'");
                        return `
                        <tr class="hover:bg-white/5 transition-all border-b border-white/5 group">
                            ...
                            <td class="p-5">
                                ...
                                <div class="text-[10px] text-slate-500 truncate max-w-[150px]">${row.subject || 'No Subject'}</div>
                            </td>
                            <td class="p-5">
...
function viewHistoryDetail(id) {
    ...
    const fallbackSubject = row && row.subject ? row.subject : '';
    selectAndMaximize('History Log', content, fallbackSubject);
}
 ...
 */
function selectAndMaximize(label, content, subject) {
    // Remove old modal if exists
    const existing = document.getElementById('maximizeModal');
    if (existing) existing.remove();

    const EF = window.EmailFormat || {};
    const parsed = EF.parseEmailContent ? EF.parseEmailContent(content) : { subject: '', body: content };
    const fallbackSubject = subject || `NeuralMail: ${label} Draft`;
    const displaySubject = parsed.subject || fallbackSubject;
    const gmailUrl = EF.buildGmailComposeUrl
        ? EF.buildGmailComposeUrl(content, fallbackSubject)
        : `https://mail.google.com/mail/?view=cm&fs=1&su=${encodeURIComponent(displaySubject)}&body=${encodeURIComponent(parsed.body || content)}`;
    const displayHtml = EF.formatEmailForPreview
        ? EF.formatEmailForPreview(content)
        ...
        <div class="bg-[#10131a] border border-white/10 p-8 rounded-[2.5rem] max-w-3xl w-full shadow-2xl relative">
            <div class="flex justify-between items-start mb-8">
                <div>
                    ...
                    <h3 class="text-2xl font-black text-white mt-2">Ready to Send</h3>
                    <p class="text-[11px] text-slate-500 mt-2 uppercase tracking-widest font-bold">Subject</p>
                    <p class="text-sm text-primary font-semibold mt-1">${EF.escapeHtml ? EF.escapeHtml(displaySubject) : displaySubject}</p>
                </div>
            ...
            <div class="email-preview-content bg-black/40 p-8 rounded-3xl border border-white/5 text-slate-200 text-sm overflow-y-auto max-h-[45vh] custom-scrollbar">
                ${displayHtml}
            </div>
            ...
            <div class="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <button id="bulkDraftModalBtn" class="py-4 bg-white/5 hover:bg-secondary/20 text-white rounded-2xl font-bold uppercase tracking-widest text-xs transition-all flex items-center justify-center gap-2">
    ...
    // Copy button - uses stored content
    ...
    document.getElementById('bulkDraftModalBtn').onclick = function() {
        useDraftInBulkSend(content);
    };
}
    ...
    try {
        ...
        if (response.ok) {
            ...
            previewArea.innerHTML = drafts.map((content, index) => {
                ...
                const parsed = EF.parseEmailContent ? EF.parseEmailContent(cleanContent) : { subject: '' };
                const previewHtml = EF.formatEmailForPreview
                    ? EF.formatEmailForPreview(cleanContent)
                    : cleanContent.replace(/</g, '&lt;').replace(/>/g, '&gt;');
                const subjectHint = parsed.subject
                    ? `<p class="text-[10px] text-primary font-semibold mb-2 truncate">${EF.escapeHtml ? EF.escapeHtml(parsed.subject) : parsed.subject}</p>`
                    : '';

                window[draftId] = cleanContent;

                return `
                <div class="glass-card glass-card-interactive p-6 rounded-[2rem] flex flex-col justify-between border border-white/5 hover:border-primary/40 transition-all duration-300 group mb-6">
                    <div>
                        ...
                        ${subjectHint}
                        <div class="email-preview-content text-[13px] text-slate-300 overflow-hidden line-clamp-[8]">
                            ${previewHtml}
                        </div>
                    </div>
...
window.useDraftInBulkSend = useDraftInBulkSend;
